...
assets['/strategic-operations-logo.png']={body:fs.readFileSync('dist/strategic-operations-logo.png').toString('base64'),type:'image/png',binary:true};
let source=fs.readFileSync('src/worker.js','utf8').replace('return env.ASSETS.fetch(request);',`{ const path=url.pathname==='/'?'/index.html':url.pathname; const asset=ASSETS[path]; return asset?new Response(asset.binary?Uint8Array.from(atob(asset.body),c=>c.charCodeAt(0)):asset.body,{headers:{'Content-Type':asset.type,'Cache-Control':'no-store','X-Content-Type-Options':'nosniff'}}):new Response('Not found',{status:404}); }`);
